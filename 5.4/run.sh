...
if [ ! -f /etc/nginx/conf.d/php-fpm.conf ]; then
  ...
  sed -i -e s:=\ apache:=\ nginx:g /opt/rh/php54/root/etc/php-fpm.d/www.conf
  sed -i -e "s:php_value\[session.save_path].*:php_value\[session.save_path] = /tmp:g" /opt/rh/php54/root/etc/php-fpm.d/www.conf
  sed -i -e "s/;cgi.fix_pathinfo=1/cgi.fix_pathinfo=0/g" /opt/rh/php54/root/etc/php.ini
